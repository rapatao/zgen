...
# Progress/info messages. Silenced when ZGEN_QUIET is set, or automatically
# while Powerlevel10k's instant prompt is active (its internal
# __p9k_instant_prompt_active is set during init and unset once the real prompt
# renders) — console I/O in that window triggers p10k's warning. Errors still
# go through -zgpute.
function -zgputi() {
    (( ZGEN_QUIET )) && return
...
# Suppress progress/info messages during startup (e.g. Powerlevel10k instant
# prompt warns about console I/O). Errors are always shown.
if [[ -z "${ZGEN_QUIET}" ]]; then
    ZGEN_QUIET=0
fi
...
# Helpers below return their result in $REPLY (zsh idiom) instead of stdout,
# so callers avoid a subshell fork per call -- notably on every shell startup.
-zgen-encode-url () {
...
-zgen-get-clone-dir() {
    ...
    if [[ -e "${repo}/.git" ]]; then
        REPLY="${ZGEN_DIR}/local/${repo:t}-${branch}"
    else
        ...
        -zgen-encode-url "${repo:h}"
        repo="${REPLY}/${reponame}"
        REPLY="${ZGEN_DIR}/${repo}-${branch}"
    fi
}
...
function zgen-clone() {
    ...
    local branch="${2:-master}"
    local REPLY
    -zgen-get-clone-url "${repo}"
    local url="${REPLY}"
    -zgen-get-clone-dir "${repo}" "${branch}"
    local dir="${REPLY}"
...
-zgen-prezto-option(){
    ...
    if (( ! ${ZGEN_PREZTO_OPTIONS[(Ie)$cmd]} )); then
...
function zgen-apply() {
    ...
    if [[ ${ZGEN_COMPILE_INIT} == 1 ]]; then
        -zgputi 'Compiling `'"${ZGEN_INIT}"'` ...';
        zcompile "${ZGEN_INIT}";
    fi

    if [[ ${ZGEN_AUTOLOAD_COMPINIT} == 1 ]]; then
        -zgputi "Initializing completions ..."
...
-zgen-get-zsh(){
    if [[ ${ZGEN_USE_PREZTO} == 1 ]]; then
        -zgen-get-clone-dir "$ZGEN_PREZTO_REPO" "$ZGEN_PREZTO_BRANCH"
    else
        -zgen-get-clone-dir "$ZGEN_OH_MY_ZSH_REPO" "$ZGEN_OH_MY_ZSH_BRANCH"
    fi
}
